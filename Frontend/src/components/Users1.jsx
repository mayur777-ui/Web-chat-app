...
export default function Users1() {
    ...
    const fetchUser = async () => {
      ...
      try {
        const response = await axios.get(
          `${USER_API_END_POINT}/getDetails/${id}`,
          {
        ...
        setConnectionList(response.data.user || []);
  ...
  const sendConnectionTo = async () => {
    ...
    try {
      await axios.post(
        `${USER_API_END_POINT}/beginconnect/${id}`,
        sendConnection,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
    ...
    let fetchNotifications = async () => {
      try {
        let res = await axios.get(
          `${Notification_API_END_POINT}/Allnotifications`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const markAsRead = async (notificationId) => {
    ...
    try {
      let res = await axios.patch(
        `${Notification_API_END_POINT}/markAsRead/${notificationId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
    ...
    const fetchUnreadCound = async () => {
      try {
        const res = await axios.get(
          `${Notification_API_END_POINT}/unreadCount`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const acceptRequest = async (senderID, notificatoinId) => {
    ...
    try {
      const res = await axios.post(
        `${USER_API_END_POINT}/acceptConnection`,
        { senderID },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
        ...
        <div className="flex items-center p-3">
          ...
          {!showNotification && (
            <div className="flex-1 ml-3 flex items-center gap-2 bg-neutral-50/70 dark:bg-neutral-800/70 backdrop-blur-lg border border-neutral-200/50 rounded-lg px-3 py-2 shadow-sm">
              ...
              />
              <Search
                className="text-neutral-500 dark:text-neutral-400 hover:text-primary-500 transition-all"
                size={20}
              />
            </div>
          )}
        ...
        >
          ...
          <div className="relative">
            ...
            >
              <img
                ...
                alt="User Avatar"
              />
              <span className="text-text-primary dark:text-text-primaryDark">
                Profile
              </span>
            </motion.div>
            {showProfile && (
          ...
          {addUser && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 10 }}
              className="absolute top-0 left-0 w-full h-full bg-neutral-50 dark:bg-neutral-800 backdrop-blur-lg border border-neutral-200/50 rounded-xl shadow-lg p-6 flex flex-col gap-6 z-50"
            >
              ...
              <div className="flex flex-col gap-2">
                <label
                  ...
                  className="text-sm font-semibold text-text-primary dark:text-text-primaryDark"
                >
                  Email
                </label>
                <input
        ...
        <ul className="flex flex-col items-center gap-6 p-4">
          <li className="relative group">
            ...
            {showProfile && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 10 }}
                className="absolute top-4 left-12 w-72 bg-gray-200 dark:bg-zinc-700 rounded-tl-none rounded-3xl p-4"
              >
          ...
          <li className="relative group">
            ...
            <motion.div
              ...
              className="absolute left-24 top-0 hidden group-hover:flex bg-neutral-800 text-white text-sm px-2 py-1 rounded-md shadow-lg z-40"
            >
              Add Connection
            </motion.div>
            {addUser && (
...
senderId)} */}
                      ...
                      <div className="p-4 bg-neutral-50 dark:bg-neutral-800 rounded-lg shadow-md mb-4">
                        <h3 className="text-md text-text-primary dark:text-text-primaryDark">
                          {notification.text}
                        </h3>
        ...
        <div className="flex-1 mt-16 md:mt-0 md:ml-20 overflow-auto bg-neutral-100 dark:bg-neutral-900">
          ...
          <div className="p-4">
            ...
            {/* Connection List */}
            {isLoading ? (
              <div className="text-center text-text-primary dark:text-text-primaryDark py-10">
                Loading...
              </div>
                ...
                {filteredConnections.map((connectOne, i) => (
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <h2 className="text-base font-medium text-text-primary dark:text-text-primaryDark">
                        {connectOne.user.name}
                      </h2>
                    </motion.div>
                  </Link>
                ))}
